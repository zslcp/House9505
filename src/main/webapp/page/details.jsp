...
<DL class="search clearfix">
  <FORM id=sform method=post action=/page/getOneHouse>
  <DT>
  <UL>
...
<c:forEach items="${}" var="list">
  <c:if test="${list.id==house.id&&list.userId==house.userId}">
    ...
    <P>户　　型：<SPAN>${list.tname}</SPAN></P>
    <P>面　　积：<SPAN>${list.floorage}<SUP>2</SUP></SPAN></P>
    <P>位　　置：<SPAN>${list.dname}->${list.sname}</SPAN></P>
    <P>联系方式：<SPAN>${list.contact}</SPAN></P></div>
    <img src="http://localhost:81/${list.path}" width="300" height="200" alt="" style="float: right">
    </DIV></DIV>
<DIV class=side>
